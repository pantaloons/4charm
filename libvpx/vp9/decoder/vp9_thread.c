...
static int pthread_create(pthread_t* const thread, const void* attr,
  ...
  (void)attr;
  *thread = (pthread_t)_beginthreadex2(NULL,   /* void *security */
...
static int pthread_join(pthread_t thread, void** value_ptr) {
  (void)value_ptr;
  return (WaitForSingleObjectEx(thread, INFINITE,FALSE) != WAIT_OBJECT_0 ||
...
static int pthread_cond_init(pthread_cond_t* const condition, void* cond_attr) {
  (void)cond_attr;
  condition->waiting_sem_ = CreateSemaphoreEx(NULL, 0, 1, NULL, 0, SEMAPHORE_MODIFY_STATE);
  condition->received_sem_ = CreateSemaphoreEx(NULL, 0, 1, NULL, 0, SEMAPHORE_MODIFY_STATE);
  condition->signal_event_ = CreateEventEx(NULL, NULL, CREATE_EVENT_MANUAL_RESET, EVENT_MODIFY_STATE);
...
static int pthread_cond_signal(pthread_cond_t* const condition) {
  int ok = 1;
  if (WaitForSingleObjectEx(condition->waiting_sem_, 0, FALSE) == WAIT_OBJECT_0) {
    // a thread is waiting in pthread_cond_wait: allow it to be notified
    ok = SetEvent(condition->signal_event_);
    // wait until the event is consumed so the signaler cannot consume
    // the event via its own pthread_cond_wait.
    ok &= (WaitForSingleObjectEx(condition->received_sem_, INFINITE,FALSE) !=
           WAIT_OBJECT_0);
  }
  return !ok;
}
...
static int pthread_cond_wait(pthread_cond_t* const condition,
  ...
  ok = (WaitForSingleObjectEx(condition->signal_event_, INFINITE,FALSE) ==
        WAIT_OBJECT_0);
