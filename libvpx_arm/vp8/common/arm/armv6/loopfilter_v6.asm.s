...
;-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ...
    ldr         r9, [src]
    add         src, src, pstep            ; p3
    ldrb        r4, [r2]                    ; blimit
    ldr         r10, [src]
    add         src, src, pstep           ; p2
    ldrb        r2, [r3]                    ; limit
    ldr         r11, [src]
    add         src, src, pstep           ; p1
    orr         r4, r4, r4, lsl #8
    ldrb        r3, [r6]                    ; thresh
    orr         r2, r2, r2, lsl #8
...
|Hnext8|
    ...
    ldr         r9, [src]
    add         src, src, pstep            ; q0
    ldr         r10, [src]
    add         src, src, pstep           ; q1
    ...
    str         r11, [src]
    add         src, src, pstep           ; store op1
    eor         r9, r9, r12                 ; *op0 = u^0x80
    str         r9, [src]
    add         src, src, pstep            ; store op0 result
    eor         r8, r8, r12                 ; *oq0 = u^0x80
    str         r8, [src]
    add         src, src, pstep            ; store oq0 result
    eor         r10, r10, r12               ; *oq1 = u^0x80
    str         r10, [src]
    add         src, src, pstep           ; store oq1
...
|hskip_filter|
    ...
    ldrne       r9, [src]
    addne       src, src, pstep            ; p3
    ldrne       r10, [src]
    addne       src, src, pstep           ; p2
    ldrne       r11, [src]
    addne       src, src, pstep           ; p1
...
|MBHnext8|
    ...
    str         r8, [src]
    add         src, src, pstep, lsl #2    ; store *op2
    add         src, src, pstep
    eor         r10, r10, lr                ; *oq2 = s^0x80
    str         r10, [src]
    add         src, src, pstep, lsl #1   ; store *oq2
...
;-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ...
    ldr         r6, [src]
    add         src, src, pstep            ; load source data
    ldrb        r4, [r2]                    ; blimit
    ldr         r7, [src]
    add         src, src, pstep
    ldrb        r2, [r3]                    ; limit
...
|vskip_filter|
    ...
    ldrne       r6, [src]
    addne       src, src, pstep            ; load source data
    ldrne       r7, [src]
    addne       src, src, pstep
    ldrne       r8, [src]
    addne       src, src, pstep
...
|MBVnext8|
    ...
    ldrb        r9, [src]
    add        src, src, pstep            ; load q2 for 1/7th difference across boundary
